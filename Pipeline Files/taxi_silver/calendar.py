...
@dp.materialized_view(
    name = "taxirides2025.silver.calendar",
    comment = "Calendar dimension with data attributes (2025)",
    table_properties = {
        "quality" : "taxirides2025.silver.calendar",
        "layer" : "silver",
        "source_format" : "csv",
...
def calendar():
    ...
    df = df.withColumn(
        "date_id",F.date_format(F.col("date"),"yyyyMMdd").cast("int")
    )
    ...
    df = df.withColumn(
        ...
        "is_weekday",
        F.when(F.col("day_of_week_num").isin([2,3,4,5,6]),True).otherwise(False),
    )
